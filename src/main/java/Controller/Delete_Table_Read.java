...
/**
 * Servlet implementation class Delete_Table_Read
 */
@WebServlet("/Delete_Table_Read")
public class Delete_Table_Read extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Delete_Table_Read() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int id= Integer.parseInt(request.getParameter("delete1"));
			...
			boolean test =manageReadExamDAO.deleteread(request, conn, id); 
					
			if(test)
			{
				manageReadExamDAO.deleteReadDetails(request, conn, id);
