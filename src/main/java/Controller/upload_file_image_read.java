...
/**
 * Servlet implementation class upload_file_image_read
 */
@WebServlet("/upload_file_image_read")
@MultipartConfig(fileSizeThreshold = 1024 * 1024 * 2, // 2MB
maxFileSize = 1024 * 1024 * 50, // 50MB
maxRequestSize = 1024 * 1024 * 50) // 50MB
public class upload_file_image_read extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public upload_file_image_read() {
        super();
	...
	for (Part part : request.getParts()) {
	    ...
	    request.setAttribute("message1",context.getRealPath("/imageRead/"));
	
	    getServletContext().getRequestDispatcher("/WEB-INF/View/Admin/addimageRead.jsp").forward(request, response);
	  }
	  /**
	  ...
	  //  File folderUpload = new File(System.getProperty("user.home") + "/Uploads");
	   // File folderUpload = new File("E:" + "/Uploads");
		  ServletContext context = request.getServletContext();
		  File folderUpload = new File(context.getRealPath("/imageRead/") );
